...
\begin{enumerate}[label=\Roman*.]  % Roman numbering for sections
    ...
    \begin{itemize}  % Bullets with circles (•)
        ...
        \item Task 7: Case study 2 – Application to earth structures (e.g.~embankments, landslides).
    \end{itemize}
...
\begin{enumerate}[label=-]
    ...
    \item \textbf{Goals:}
    \begin{itemize}
        \item Data collection: Gather high-quality time-series data from satellite imagery, ground sensors, etc.
...
\fbox{%
  \begin{minipage}[t]{\textwidth}  % Define a caixa para ocupar a largura total da página
    Output (Tasks 4 and 5)  – Paper/Article: Development of a machine learning-based toolbox for detecting movement patterns in geotechnical structures: A comparative analysis.
    \\[0.5em]
...
Test the numerical toolbox on real-world data from concrete structures, such as dams, to evaluate its performance in detecting movement patterns and structural behaviour.

\textbf{Task 7: Case study 2 – application to earth structures (e.g., embankments)} 

Apply the developed numerical toolbox to earth structures, such as embankments, for validation and comparison of its effectiveness in monitoring ground displacement and structural stability.
...
\begin{table}[h!]
    ...
    \hline
    \textbf{} & \textbf{Semester} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{8} \\ %\hline
    \hhline{|-|-|-|-|-|-|-|-|-|-|}
    \multirow{3}{*}{I} 
...
%\section{References}
\setlength{\bibitemsep}{1em} % ou outro valor, ex: 0.5em, 1.2em
\printbibliography
